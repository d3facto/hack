...
export const Header = ({ connected }) => {
    ...
    return <div style={{ height: '80px', textAlign: 'center', padding: '10px', backgroundColor: 'white', fontSize: '2rem' }}>
        ...
        { connected == undefined ?
            <button style={{ position: 'absolute', right: '20px' }} className='login-button' onClick={redirect}>Login</button> :
            <button style={{ position: 'absolute', right: '20px' }} className='login-button' onClick={logout}>Logout</button> }
    </div>
}